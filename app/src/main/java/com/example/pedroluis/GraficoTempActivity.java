...
public class GraficoTempActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        graph_1D.setOnClickListener(view -> {
            cont = 0;
            barEntries.clear();
            publish(idEstufa, "Smart_Farm/" + mqttHelper.getClientId() + "/Sensores/Temp/Grafico1D");
        });

        graph_1S.setOnClickListener(view -> {
            cont = 0;
            publish(idEstufa, "Smart_Farm/" + mqttHelper.getClientId() + "/Sensores/Temp/Grafico1S");
        });

        graph_1M.setOnClickListener(view -> {
            cont = 0;
            publish(idEstufa, "Smart_Farm/" + mqttHelper.getClientId() + "/Sensores/Temp/Grafico1M");
        });
    ...
    private void JoaoMqtt() {
        ...
        mqttAndroidClient.setCallback(new MqttCallbackExtended() {
            ...
            public void messageArrived(String topic, MqttMessage mqttMessage) throws Exception {
                ...
                if (topic.equals("Smart_Farm/"+mqttHelper.getClientId()+"/Sensores/Temp/MediasHoras")) {
                    val_inst = mqttMessage.toString();
                    valor = Float.parseFloat(val_inst);
                    BarEntry barEntry = new BarEntry(cont, valor);
                    barEntries.add(barEntry);
                ...
                if (topic.equals("Smart_Farm/"+mqttHelper.getClientId()+"/Sensores/Temp/MediasDias")) {
                    val_inst = mqttMessage.toString();
                    valor = Float.parseFloat(val_inst);
                    BarEntry barEntry = new BarEntry(cont, valor);
                    barEntries.add(barEntry);
